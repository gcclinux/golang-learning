...
func main() {
	...
	fmt.Printf("Your Name is: %s. You are %d years old. And your favourite number is %g. Owns a dog: %t\n",
		user.UserName,
		user.Age,
...
func readBool(s string) bool {
	...
	for {
		...
		if char != 'n' && char != 'N' && char != 'y' && char != 'Y' {
			fmt.Println("Please type in the correct answer (y/n)")
		} else {
			if char == 'n' || char == 'N' {
				return false
			} else {
